...
class BudgetApiKtTest : ServerTest() {
    ...
    fun testBudgetGet() {
        val id: String
        RestAssured.given()
            .jsonBody(AuthorRecordRequest("Московкин Олег Романович"))
            .post("/author/add")
        ...
        addRecord(BudgetRecordRequest(2020, 5, 10, BudgetType.Приход, id))
        ...
        RestAssured.given()
            ...
            .toResponse<BudgetYearStatsResponse>().let { response ->
                ...
                Assert.assertEquals(id, response.items[0].first.author)
                Assert.assertEquals(null, response.items[1].first.author)
            }
    }
    ...
    fun testBudgetPagination() {
        ...
        RestAssured.given()
            .queryParam("pagingLimit", 3)
            .queryParam("pagingOffset", 1)
    ...
    fun testStatsSortOrder() {
        ...
        RestAssured.given()
            ...
            .toResponse<BudgetYearStatsResponse>().let { response ->
                println(response.items)
                Assert.assertEquals(30, response.items[0].first.amount)
                Assert.assertEquals(5, response.items[1].first.amount)
                Assert.assertEquals(400, response.items[2].first.amount)
                Assert.assertEquals(100, response.items[3].first.amount)
                Assert.assertEquals(50, response.items[4].first.amount)
            }
    }
    ...
    private fun addRecord(record: BudgetRecordRequest) {
        RestAssured.given()
            ...
            .toResponse<BudgetRecord>().let { response ->
                ...
                Assert.assertEquals(record.author, response.author)
            }
    }
}